...
bool testRsa(bool toModifyMessage)
{
	KeypairContext* privateKeyContext = NULL;
	KeypairContext* publicKeyContext = NULL;

	if (!readRsaKeypair("bob.key", "bobkey", "bob.crt", &privateKeyContext, &publicKeyContext))
	{
...
int main(int argc, char** argv)
{
	...
	if (testRsa(false /* do not change message in transit*/) && !testRsa(true /* change message in transit*/))
	...
	if (testCertificateChecking("bob.crt", "rootCA.crt", "Bob.com") && !testCertificateChecking("bob.crt", "rootCA.crt", "Boby.com") && !testCertificateChecking("bob.crt", "alice.crt", "Bob.com"))
